...
if (isset($_GET['page']) && $_GET['page'] == 'wp-leaves/leaves.php'){
	wp_enqueue_script('jquery');	
	wp_register_script('my-upload', plugins_url("wp-leaves/script.js"), array('jquery','media-upload','thickbox'));
	wp_enqueue_script('my-upload');
	wp_enqueue_style('thickbox');
	wp_register_style('myStyleSheets', plugins_url("wp-leaves/style.css"));
...
function wp_leaves_createflashcode($name,$id,$heightIn,$widthIn){
    $rand = rand(5, 99); 
	$flashtag .= '<div style = "float:left; width:'.$widthIn.'px; height:'.$heightIn.'px; margin:5px; "><script type="text/javascript" src="'.plugins_url("wp-leaves/swfobject/swfobject.js").'" charset="utf-8"></script><script type="text/javascript" src="'.plugins_url("wp-leaves/swfobject/swfaddress.js").'" charset="utf-8"></script><script type="text/javascript">
								var flashvars = {queryPath:"'.plugins_url('wp-leaves/query.php?id='.$rand ).'",BannerID:"'.(string)$id.'"};   
								var params = {};
								var attributes = {};
								...
								swfobject.embedSWF("'.plugins_url("wp-leaves/fallingLeaves.swf?id=".$rand ).'", "'.$name.'-'.$id.'", "'.$widthIn.'px", "'.$heightIn.'px", "9.0.0", "'.plugins_url("wp-leaves/swfobject/expressInstall.swf").'", flashvars, params, attributes);</script><div id="'.$name.'-'.$id.'"></div></div>';
	return $flashtag;
}
